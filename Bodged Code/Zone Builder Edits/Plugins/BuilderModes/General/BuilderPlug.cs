	...
	public class BuilderPlug : Plug
	{
		...
		private void ExportToObj() 
		{
			foreach (Thing t in General.Map.Map.Things)
			{
				...
				t.DetermineSector();
				float floorheight = (t.Sector != null ? Sector.GetFloorPlane(t.Sector).GetZ(t.Position) : 0);
				var z = (int)Math.Round(t.Position.z + floorheight);
				Console.WriteLine($"{ti.Title}|{t.Position.x * 2.25}|{z * 2.25}|{-t.Position.y * 2.25}|{t.AngleDoom}");
			}
			return;
		...
		//[BeginAction("exportthings")]
		private void ExportThings()
		{
